...
export class Ethereum implements Blockchain {
  ...
  static async create(nodeEndpoint: string | Object): Promise<Ethereum> {
    ...
    ethereum.decimals = {
      ...ethereum.contractAddresses[ethereum.networkType]
        .nonStandardTokenDecimals
    };
  ...
  async getRates(
    ...
    const tokenRatesPerAmount = await getRatesSafe(
      this,
      addressPaths,
    ...
    return tokenRatesPerAmount.map(tokenRates =>
      tokenRates.map(tokenRate => helpers.fromWei(tokenRate, targetDecimals))
    );
  }
  ...
  async getConversionEvents(
    ...
    return await converterEvents.getConversionEvents(
      this.web3,
      this.decimals,
      token.blockchainId,
      fromBlock,
  ...
  async getTokenRateEvents(
    token: Token,
    fromBlock: number,
    toBlock: number
  ): Promise<TokenRateEvent[]> {
    return await converterEvents.getTokenRateEvents(
      this.web3,
      this.decimals,
      token.blockchainId,
      fromBlock,
  ...
  async getTokenRateEventsByTimestamp(
    token: Token,
    fromTimestamp: number,
    toTimestamp: number
  ): Promise<TokenRateEvent[]> {
...
export const getRates = async function(
  ...
  return Array(amounts.length)
    .fill("0")
    .map((_, i) => {
      const _returnData = returnData.slice(
        i * paths.length,
        i * paths.length + paths.length
      );
      return _returnData.map(item =>
